...
class Painel extends JPanel {
	...
	public void paint(Graphics g) {
		...
		/* Ponto */
		case 3: {
			f.inserir(new Figura(new Ponto2D(pointX - 1, pointY - 1), new Ponto2D(3, 3), 3), 500);

			break;
		}
		/* Linha */
		case 4: {
			f.inserir(new Figura(new Ponto2D(originX, originY), new Ponto2D(radiusX, radiusY), 4), 500);
		...
		case 5: {
			/*
			 * if (originX < radiusX){ if (originY < radiusY) { if (radiusX-originX >
			 * radiusY-originY) { f[numFig].setOrigem(new Ponto2D(originX, originY));
			 * f[numFig].setFim(new Ponto2D(radiusX-originX, radiusX-originX)); }
			 * 
			 * 
			 * else { f[numFig].setOrigem(new Ponto2D(originX, originY));
			 * f[numFig].setFim(new Ponto2D(radiusY-originY, radiusY-originY)); } }
			 * 
			 * else { if (radiusX-originX > originY-radiusY) { f[numFig].setOrigem(new
			 * Ponto2D(originX, radiusY)); f[numFig].setFim(new Ponto2D(radiusX-originX,
			 * radiusX-originX)); }
			 * 
			 * else { f[numFig].setOrigem(new Ponto2D(originX, radiusY));
			 * f[numFig].setFim(new Ponto2D(originY-radiusY, originY-radiusY)); } } } else{
			 * if (originY < radiusY) { if (originX-radiusX > radiusY-originY) {
			 * f[numFig].setOrigem(new Ponto2D(radiusX, originY)); f[numFig].setFim(new
			 * Ponto2D(originX-radiusX, originX-radiusX)); }
			 * 
			 * else { f[numFig].setOrigem(new Ponto2D(radiusX, originY));
			 * f[numFig].setFim(new Ponto2D(radiusY-originY, radiusY-originY)); }
			 * 
			 * } else { if (originX-radiusX > originY-radiusY) { f[numFig].setOrigem(new
			 * Ponto2D(radiusX, radiusY)); f[numFig].setFim(new Ponto2D(originX-radiusX,
			 * originX-radiusX)); }
			 * 
			 * else { f[numFig].setOrigem(new Ponto2D(radiusX, radiusY));
		...
		/* Triangulo */
		case 6: {
			f.inserir(new Figura(new Ponto2D(pontoAX, pontoAY), new Ponto2D(pontoBX, pontoBY), 4), 500);

			f.inserir(new Figura(new Ponto2D(pontoBX, pontoBY), new Ponto2D(pontoCX, pontoCY), 4), 500);

			f.inserir(new Figura(new Ponto2D(pontoCX, pontoCY), new Ponto2D(pontoAX, pontoAY), 4), 500);
